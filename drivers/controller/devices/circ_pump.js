...
class CircpumpDevice extends Device {
  _api = null;
  _pollList = [];
  _pollTimer = null;
  _settings = {};
   ...
   */
  async onInit() {
    console.log("Circpump device initiated.");
  ...
  assemblePollList() {
    ...
    this._pollList.push({
      capability: "automan",
      modbus_item: ModbusModel.CIRCPUMP_AUTO,
    });
    this._pollList.push({
      capability: "onoff",
      modbus_item: ModbusModel.CIRCPUMP_ONOFF,
    });
    this._pollList.push({
      capability: "output",
      modbus_item: ModbusModel.CIRCPUMP_OUTPUT,
    });
    this._pollList.push({
      capability: "ontime",
      modbus_item: ModbusModel.SP_CIRCPUMP_ON,
    });
    this._pollList.push({
      capability: "offtime",
      modbus_item: ModbusModel.SP_CIRCPUMP_OFF,
    });
  ...
  addCapabilityListeners() {
    ...
    this.registerCapabilityListener("automan", async (value) => {
      this._api
        .writeSingleCoil(ModbusModel.CIRCPUMP_AUTO.address, value)
        .catch((err) => {
          console.log(err);
        });
    });
    ...
    this.registerCapabilityListener("onoff", async (value) => {
      this._api
        .writeSingleCoil(ModbusModel.CIRCPUMP_ONOFF.address, value)
        .catch((err) => {
          console.log(err);
        });
    });
        ...
        this._api
          ...
          .then(async (res) => {
            ...
            if (item.capability == "onoff") {
              if (this.getCapabilityValue("output") == res) {
                this.setCapabilityValue(item.capability, res);
              }
            }
            // Update the settings if timer is changed elsewhere.
            if (item.capability == "ontime") {
              if (this._settings["circpump_on"] != res) {
                await this.setSettings({ circpump_on: res }).catch((err) =>
                  console.log(err)
                );
                this._settings = this.getSettings();
              }
            ...
            if (item.capability == "offtime") {
              if (this._settings["circpump_off"] != res) {
                //console.log("Result from poll: ", res);
                //this._settings["growlights_off"] = res;
                await this.setSettings({ circpump_off: res }).catch((err) =>
                  console.log(err)
                );
                this._settings = this.getSettings();
              }
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes("circpump_on")) {
      console.log("Circpump timer changed. Writing to register.");
      await this._api
        .writeSingleRegister(
          ModbusModel.SP_CIRCPUMP_ON.address,
          newSettings["circpump_on"]
        )
        .then((res) => {
          console.log(res);
        })
    ...
    if (changedKeys.includes("circpump_off")) {
      await this._api
        .writeSingleRegister(
          ModbusModel.SP_CIRCPUMP_OFF.address,
          newSettings["circpump_off"]
        )
        .then((res) => {
          console.log(res);
        })
